#!/usr/bin/env bash
# fixup-helper.sh — A quick way to generate fixup-mountpoints for devices

# Variables (user defined)
HALIUM_ROOT="$HOME/halium"
...
if ! cd "$HALIUM_ROOT" &> /dev/null; then
	msg "ERROR: Couldn't enter \$HALIUM_ROOT ('$HALIUM_ROOT')!"
	msg "Please re-configure the script variable!"
	exit 1
fi
...
# 5. Wait for a device in recovery mode...
...
msg "We're done here; mountpoints for your device can be found in '\$HALIUM_ROOT/halium/hybris-boot/mountpoints-for-$boardname.txt'"
msg "Place the generated mountpoints in '\$HALIUM_ROOT/halium/hybris-boot/fixup-mountpoints' among all other devices"